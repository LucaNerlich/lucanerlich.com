...
description: My zsh + oh-my-zsh + starship terminal setup, Ghostty config, daily CLI tools (fzf, zoxide, lazygit, lazydocker), and the .zshrc / PowerShell profiles I use day to day. Version management (mise) and AI agent harnesses now live in their own posts.
tags: [shell, zsh, terminal, tooling, productivity]
keywords: [zsh, oh-my-zsh, starship, ghostty, zshrc, powershell profile, terminal setup, fzf, zoxide, lazygit, lazydocker, macshot]
---
...
Tool versions for node, java, maven, python, and friends are no longer managed
by nvm or hand-pinned `JAVA_HOME` exports -- [mise](https://mise.jdx.dev)
(mise-en-place) handles all of them, per directory. It got big enough to earn
its own post, including how global vs. per-project config resolves:

**See [Local Version Management with mise](./mise.md).**

## AI harness

The terminal coding-agent harnesses I run -- [opencode](https://opencode.ai/v2/),
[pi](https://pi.dev), and [herdr](https://herdr.dev) -- now have their own
post too:

**See [AI Agent Harnesses](../ai/agent-harnesses.md).**
