...
import EventBubblingDemo from '@site/src/components/demos/javascript/EventBubblingDemo';
...
**Important:** you must pass the same function reference. If you used capture mode, the capture setting must match too:
...
If a scroll or touch handler never calls `preventDefault()`, use `{ passive: true }` so the browser can keep scrolling
smoothly. Do not use passive listeners when you need to cancel the event.
...
Click the nested button and watch the bubble path. Toggle `stopPropagation` to cut it short:
