...
import DomWorkshopDemo from '@site/src/components/demos/javascript/DomWorkshopDemo';
...
Practice the core mutations -- `textContent`, `classList`, `createElement` / `append`, and `remove`:
...
The NodeList returned by `querySelectorAll` is **static**: it does not update if matching elements are added or removed
later. Older methods like `getElementsByClassName` return live collections that update automatically.
...
For URLs, the property (`link.href`) resolves to an absolute URL, while `getAttribute("href")` returns the literal
attribute value from the HTML.
...
container.replaceChildren(); // modern, clean way to remove all children

// Also works, but re-parses HTML and drops event listeners on children
container.innerHTML = "";

// Older loop equivalent
...
| `firstElementChild`  | `firstChild`  | `firstChild` can be a text node or comment    |
| `parentElement`      | `parentNode`  | `parentNode` can be a non-element node        |
| `nextElementSibling` | `nextSibling` | `nextSibling` can be a text node or comment   |

Use the **Element** versions (left column) unless you specifically need to work with text nodes.

## Document fragments

When adding many elements, using a `DocumentFragment` reduces repeated DOM insertions:
...
Next up: [Events & Interactivity](./09-events.mdx) - making your page respond to user actions.
