...
import StorageDemo from '@site/src/components/demos/javascript/StorageDemo';
...
- **Avoid secrets and tokens**: localStorage is readable by any script on the page.

## Handling quota and access errors

Web Storage is convenient, but it is not guaranteed to be available. Merely accessing `window.localStorage` or
`window.sessionStorage` can throw a `SecurityError` when storage is blocked, for example when the user blocks site data
or the page runs in a sandboxed iframe. Writing can also fail: `setItem` can throw a `DOMException` named
`QuotaExceededError` when the origin has used too much storage.

Use a feature detect before relying on storage:
...
When a write fails, check `error.name`:
...
Limits are usually around 5 MiB per origin, but the exact limit varies by browser. In private browsing, storage normally
works for the session and is discarded when the private session ends. Older Safari versions threw on `setItem` in private
mode, which is another reason to feature-detect.

Web Storage is synchronous and blocks the main thread. Keep values small, and use IndexedDB for larger data.
...
Save a note in each store, then reload the page or open a new tab to compare persistence:
...
The `storage` event fires in **other tabs** when a value changes -- not in the tab that made the change. Use it to keep
UI in sync across tabs.
...
// Trigger this from another tab or window:
localStorage.setItem("theme", "light");
```

Result (in another open tab that registered the listener):

```text
theme changed to light
```

To react to storage changes within the same tab (document), dispatch a custom event after `setItem`
- `BroadcastChannel` does not help here either, since a channel never receives its own messages; it is
useful for synchronizing with *other* same-origin tabs, windows, frames, or workers.
