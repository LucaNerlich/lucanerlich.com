...
import IntlFormatDemo from '@site/src/components/demos/javascript/IntlFormatDemo';
...
Change locale, currency, amount, and date -- the same values format differently per audience:
...
Note that `Intl` output often contains non-breaking spaces (U+00A0) or narrow no-break spaces (U+202F) instead of
regular spaces. They look identical, but a string comparison such as `formatted === "USD 1,234.56"` with a typed space
fails. Avoid asserting exact formatted strings in tests, or normalize whitespace first.
