...
import TransitionDemo from '@site/src/components/demos/css/TransitionDemo';
...
Toggle a card state and compare durations and easing curves:
...
Moves an element. `translateX` shifts along the x-axis, `translateY` along the y-axis, and `translate(x, y)` moves in
...
This is the CSS half of a scroll-reveal pattern. In practice, the `.visible` class is usually added by JavaScript
(for example with `IntersectionObserver`) or by newer scroll-driven animation APIs.
