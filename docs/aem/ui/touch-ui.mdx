---
title: Touch UI (Author UI)
sidebar_position: 1
tags: [aem, ui, authoring]
---
...
:::info
The modern AEM authoring interface is officially called the **Touch UI** (also referred to as the
Author UI). It replaced the legacy Classic UI and is the interface used across AEM 6.5 LTS and
AEM as a Cloud Service.
:::

:::note
Classic UI is deprecated in AEM 6.5 and is not available in AEM as a Cloud Service. Authoring
customisations should target the Touch-enabled authoring UI and load through authoring clientlib
categories such as `cq.authoring.editor`, `cq.authoring.editor.hook`, or `cq.authoring.dialog`.
:::

## Add the Styles Plugin to the RTE

The RTE Styles plugin allows an author to add a specific CSS class to a text selection.
AEM wraps the selection with a tag:
...
![RTE styles dialog](/images/aem/ui/rte-styles.png)


HTML Output:
![RTE styles HTML output](/images/aem/ui/rte-styles-html.png)
...
Note that the [Sling Resource Merger](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/platform/sling-resource-merger) pulls in additional nodes from the parent Text Core Component dialog.
Therefore we can (mostly) add only our new Styles Plugin-related nodes.
