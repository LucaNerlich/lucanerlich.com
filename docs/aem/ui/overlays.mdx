...
The Sling Resource Merger is active by default in AEM for merged virtual paths such as
`/mnt/overlay` and `/mnt/override`. Normal Sling resource resolution still uses the search path
(`/apps` before `/libs`); Granite UI consoles request their definitions through the merger so
matching `/apps` nodes can merge with product nodes under `/libs`:
...
:::warning[AEM as a Cloud Service]
`/libs` is immutable on AEM as a Cloud Service. Keep overlays in `/apps`, keep them minimal, and
only overlay supported public or abstract product areas. Nodes marked with mixins such as
`granite:InternalArea` or `granite:FinalArea` are implementation details and can change without
        ...
        if (ev.layer === 'Edit') {
            ...
            author.EditorFrame.editableToolbar.config.actions.ANALYTICS = {
                ...
                handler: function(editable) {
                    ...
                    window.open('/mnt/overlay/dam/gui/content/assets/analysisreport.html?path='
...
Toolbar JavaScript should be loaded via a page-editor clientlib category such as
`cq.authoring.editor`:
...
| Toolbar action JS doesn't load | Verify the clientlib category is `cq.authoring.editor` |
...
- [Overlays Documentation](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/platform/overlays)
- [Sling Resource Merger](https://sling.apache.org/documentation/bundles/resource-merger.html)
- [Customising the AEM Sites Console](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/platform/overlays)
