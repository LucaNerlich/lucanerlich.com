...
// The !default flag lets tenant SCSS files override these values by defining
// them before this file is imported.
$color-primary: #0066cc !default;
$color-secondary: #333333 !default;
$color-background: #ffffff !default;
$color-text: #1a1a1a !default;
...
const CLIENT_LIBS = SITES.map(siteName => ({
    ...
    assets: {
        ...
        resources: {
            cwd: `clientlib-${siteName}`,
            files: ['**/*.*'],
            flatten: false,  // preserve resources/fonts/... and resources/images/... paths
            ignore: ['**/*.js', '**/*.css'],
        },
    },
}));
...
:::info[Preserve resource paths]
Keeping `flatten: false` on the resources section preserves folders such as `fonts/` and `images/`
under the generated clientlib's `resources/` folder. This ensures that relative paths like
`url('../resources/fonts/roboto.woff2')` continue to resolve after clientlib generation.
:::
...
:::tip[Packaged alternative to hand-rolling this]
The hand-rolled `vite.config.js` above and the matching `aem-clientlib-generator` config both need to
be kept in sync per tenant. [`@aemvite`](../aemvite.mdx) packages this exact pattern: declare
one `clientlibs` entry per tenant in a typed `aem.config.mjs`, and both the Vite build and the
byte-identical `.content.xml` / `js.txt` / `css.txt` emission are handled for you - see
[Multiple clientlibs for multi-tenant setups](../aemvite.mdx#71-multiple-clientlibs-for-multi-tenant-setups).
:::
...
```javascript title="clientlib.config.js (excerpt)"
{
    name: 'clientlib-site-X',
...
- [@aemvite: Migrate ui.frontend from Webpack to Vite](../aemvite.mdx) - packaged Vite toolchain covering the per-tenant clientlib pattern shown above
