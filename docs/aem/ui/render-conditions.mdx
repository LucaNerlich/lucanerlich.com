...
the UI is rendered, so hidden elements never reach the browser. They are not an authorization
boundary: protect content and write operations with ACLs, servlets, Sling Models, and workflow
...
Show an element only when a Granite feature flag is enabled:

```xml
<granite:rendercondition
    jcr:primaryType="nt:unstructured"
    sling:resourceType="granite/ui/components/coral/foundation/renderconditions/feature"
    feature="com.example.authoring.feature"/>
```
...
<granite:rendercondition
    ...
    <condition1
        jcr:primaryType="nt:unstructured"
        sling:resourceType="granite/ui/components/coral/foundation/renderconditions/feature"
        feature="com.example.authoring.featureA"/>
    <condition2
        jcr:primaryType="nt:unstructured"
        sling:resourceType="granite/ui/components/coral/foundation/renderconditions/feature"
        feature="com.example.authoring.featureB"/>
</granite:rendercondition>
```
...
| `.../renderconditions/feature` | Check a Granite feature flag |

All live under `granite/ui/components/coral/foundation/renderconditions/`. The simple condition uses
Granite's expression resolver, so expressions can refer to values such as
`${requestPathInfo.suffix}`, `${param.item}`, and Granite functions such as
`${granite:relativeParent(requestPathInfo.suffix, 1)}`.
...
|-----------|---------------|-------|----------|
| **Render Condition** | Server-side, before dialog renders | Granite UI (dialogs, admin consoles) | UI hiding only; enforce access separately |
| **`data-sly-test`** | Server-side, during HTL rendering | HTL templates (component rendering) | UI/output hiding only; enforce access separately |
| **`sling:hideResource` / `sling:hideChildren`** | Request-time, during Sling Resource Merger resolution | Dialog inheritance and overlays | Removes inherited UI nodes; not a permission check |
...
- [Granite UI Render Conditions API Reference](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/renderconditions/index.html) - built-in render condition components
- [Render Conditions (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/components/hide-conditions) - hide conditions and rendering control
