...
The underlying `granite/ui/components/coral/foundation/renderconditions/feature` component is
documented in the Granite UI API reference:
[AEM 6.5 Granite UI API reference](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/renderconditions/feature/index.html).
:::
...
public class SmartCropCheckServlet extends SlingSafeMethodsServlet {
    ...
    private boolean isValidAssetPath(String path) {
        return path != null
                && (path.equals(CONTENT_DAM_PREFIX) || path.startsWith(CONTENT_DAM_PREFIX + "/"))
                && !path.contains("..");
    }
}
```
