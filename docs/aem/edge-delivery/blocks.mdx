---
title: Blocks (the component model)
...
sequenceDiagram
    ...
    Page->>aem: loadEager()
    aem->>Block: decorate(blocks in first section)
    Block-->>aem: returns
    aem->>Page: wait for LCP image

    Page->>aem: loadLazy()
    aem->>Block: import + decorate(remaining sections' blocks)
    aem->>Page: header, footer, lazy-styles.css, fonts

    Page->>aem: loadDelayed()
    Note over aem: setTimeout(..., 3000)
    aem->>Browser: import delayed.js
```

| Phase | When | Used for |
|-------|------|----------|
| **Eager** | Before LCP | Blocks in the first section (hero, above-the-fold content) |
| **Lazy** | Right after LCP | All remaining sections and blocks, header, footer, lazy styles, fonts |
| **Delayed** | About 3 seconds after load | `delayed.js`: analytics, chat, marketing tags |

You don't usually pick the phase per block - `aem.js` decides based on the section the block
sits in. Blocks that should only do expensive work when visible (maps, videos, embeds) can add
their own `IntersectionObserver` inside `decorate()`. You can override the phases in
`scripts/scripts.js` if needed.
...
The runtime applies `style` values as space-separated classes on the containing
section element, and exposes any other keys as `dataset` on that same section. From a
block inside the section:

```javascript
const section = block.closest('div.section');
const audience = section?.dataset.audience; // "mobile"
```

Handy for blocks that need to adapt to surrounding context.
...
The [aem-boilerplate](https://github.com/adobe/aem-boilerplate) (a handful of starter
blocks) and the [Block Collection](https://www.aem.live/developer/block-collection)
(source: [aem-block-collection](https://github.com/adobe/aem-block-collection)) ship
