...
| Custom domain shows stale content | BYO CDN missing push invalidation | Configure push invalidation - see [Customizing](./customizing.mdx#bring-your-own-cdn) |
...
| Deleted page is still live | Deleting the source document does not unpublish it | Unpublish (Sidekick or Admin API) before deleting; run an [Admin API drift audit](./admin-api.mdx#audit-drift-between-source-and-live) |
...
- Check performance on every PR (see [Measuring on every PR](./performance.mdx#measuring-on-every-pr)).
...
- [aem-block-collection](https://github.com/adobe/aem-block-collection) - source of the Block Collection
...
- [EDS for Forms](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/build-forms/overview)
