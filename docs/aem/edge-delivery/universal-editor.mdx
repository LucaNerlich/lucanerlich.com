...
| `data-aue-model` | Component model used for component insertion/editing |
...
Document-based authoring does not require these attributes. For UE, the source comes
pre-instrumented from AEM or from the UE/XWalk authoring pipeline, and your block
decoration must **preserve** the attributes when restructuring the DOM.
...
The site's `fstab.yaml` points at the Edge Delivery servlet on AEM author (the layout used by
the [aem-boilerplate-xwalk](https://github.com/adobe-rnd/aem-boilerplate-xwalk) template):
...
Sites managed through the [Configuration Service](./admin-api.mdx#config---configuration-service)
store the same information in the `content.source` section of the site configuration instead
of `fstab.yaml`. `paths.json` then maps AEM paths such as `/content/mysite/` to public URLs (see
[Development](./development.mdx#pathsjson)).

The rendered page also needs the Universal Editor connection metadata, usually in
`head.html`:
...
UE is launched from the AEM Sites console (or via a deep link) at
`https://experience.adobe.com/#/@{tenant}/aem/editor/canvas/{path}` and points at the
same AEM author instance. The editor then loads the EDS preview of the page being
edited inside an iframe and layers the authoring UI on top.
...
- Adobe docs: [Universal Editor introduction](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/implementing/developing/universal-editor/introduction)
