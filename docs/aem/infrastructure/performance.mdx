...
If Oak chooses the wrong index, you can limit index selection with Oak's `option(index tag ...)`
hint:
...
If a service is only needed in one code path, look it up lazily via the OSGi `BundleContext` instead of injecting it at model construction time. This avoids the cost of the service's `@Reference` chain being resolved on every model adaptation, even when the code path is never taken.

```java
// BAD - SearchService is injected (and its @Reference chain resolved) on every adaptation
@OSGiService
private SearchService searchService;

// GOOD - resolve only when the search path is actually taken
public List<Result> search(String term) {
    ...
    ServiceReference<SearchService> ref = ctx.getServiceReference(SearchService.class);
    if (ref == null) {
        return List.of();
    }
    SearchService service = ctx.getService(ref);
    if (service == null) {
        return List.of();
    }
    try {
...
```json title="com.adobe.granite.ui.clientlibs.impl.HtmlLibraryManagerImpl.cfg.json"
{
...
```json title="org.apache.jackrabbit.oak.query.QueryEngineSettingsService.cfg.json"
{
...
- [AEM Performance (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/deploying/configuring/configuring-performance) - server-side performance tuning guide
- [Oak Query Performance](https://jackrabbit.apache.org/oak/docs/query/query-engine.html) - index types and query optimisation
- [AEMaaCS Performance (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/manage-logs) - cloud monitoring and log analysis
