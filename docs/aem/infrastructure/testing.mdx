...
| Dispatcher config | **AEM Dispatcher SDK validator** (`bin/validate.sh` / `bin/validator`) | Validates the AEMaaCS Dispatcher source tree offline |
...
"$DIS_SDK/bin/validator" full dispatcher/src
```

Wire it into CI as a pre-merge gate. The SDK ships the `validator` binary under `bin/`;
`full` runs the same combined checks that Cloud Manager runs on every deployment. See the
dedicated [Dispatcher Configuration](./dispatcher-configuration.mdx) page for authoring
guidance and auto-invalidation rules.
...
- [ ] Exercise Dispatcher by requesting a page twice - second request should be a cache hit
      (check `dispatcher.log`, the cache directory, or `X-Cache-Info` only if `/info "1"` is enabled and you send `X-Dispatcher-Info`).
...
- [wcm.io AEM Mocks](https://wcm.io/testing/aem-mock/) - `AemContext` JUnit integration for Sling Models and OSGi services
- [AEM Testing (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/development-tools) - developer tooling and testing setup
- AEMaaCS UI Testing - Selenium-based UI tests in Cloud Manager pipelines (custom functional testing step)
