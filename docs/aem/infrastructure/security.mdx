...
:::info[AEMaaCS vs AEM 6.5]
The factory PID and JSON shape are identical on both runtimes - only the delivery differs.

- **AEMaaCS:** ship the `.cfg.json` in `ui.config` and create the service users plus their ACLs
  via `repoinit` statements in `org.apache.sling.jcr.repoinit.RepositoryInitializer` configs.
  `admin`-level users cannot be created, and filesystem writes outside the repository don't
  survive restarts.
- **AEM 6.5 / on-prem:** the same `.cfg.json` works in `ui.config`, or you can create the mapping
  through the OSGi Console (`/system/console/configMgr`) on a running instance. Service users can
  also be created manually in the Users admin UI.
:::
...
HTL does not provide a magic `@csrf-token` expression. For server-rendered forms, expose the
token from Java (for example a Sling Model using `com.adobe.granite.csrf.CSRFTokenProvider`) and
render it as an escaped attribute:

```html
<sly data-sly-use.form="com.myproject.core.models.CsrfFormModel" />
<form method="POST" action="/content/my-project/submit">
    <input type="hidden" name=":cq_csrf_token" value="${form.token @ context='attribute'}" />
...
```java
import com.adobe.granite.csrf.CSRFTokenProvider;
...
public class CsrfFormModel {
    ...
    public String getToken() {
        return csrfTokenProvider.getToken(request);
    }
}
```
...
Anonymous publish requests may not receive a CSRF token. If a public form needs to POST from
publish, test that flow explicitly and combine CSRF with an appropriate authentication or anti-abuse
mechanism for the endpoint.
...
| `/libs/*` | AEM platform code (except explicitly allow-listed paths, e.g. `/libs/granite/csrf/token.json`) |
...
Note that `/etc.clientlibs/*` should be **explicitly allowed**, not blocked -- it serves the versioned, proxied client-side JS and CSS bundles that the site depends on.

`/libs/granite/csrf/token.json` is per-session and must be excluded from Dispatcher, CDN, and
browser caching (deny it in your [cache rules](./dispatcher-configuration.mdx#cache-rules) and/or
send `Cache-Control: no-store`) - a cached token would be handed out to unrelated visitors and
break CSRF validation for all of them.
...
- [AEM Security Checklist (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/security/security-checklist) - pre-go-live security review
- [AEMaaCS Security Overview (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/security/cloud-service-security-overview) - cloud-native security model
- [OWASP Top 10](https://owasp.org/www-project-top-ten/) - web application security risks reference
