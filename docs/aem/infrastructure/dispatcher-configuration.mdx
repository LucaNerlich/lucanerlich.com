...
An end-to-end check verifies that the Dispatcher can reach the publish tier and that the publish tier responds. AEM ships a lightweight probe at `/libs/granite/core/content/login.html` -- allow it through the filter:
...
Filter rules do not have a client-header condition, so if you want to restrict the probe to the load-balancer subnet do it at the Apache `<Location>` level (`Require ip ...`) or block the path from the CDN.
