...
<Location /libs/granite/core/content/login.html>
    Require ip 10.0.0.0/8   # load-balancer / internal subnet only
</Location>
```

Without the `<Location>` restriction, this filter rule alone makes the probe reachable from the public internet. If your load balancer's source IPs are not stable (e.g. a managed CDN), allowlist the CDN's published IP ranges instead, or move the check behind a private/internal listener.
