...
│   ├── rewrites/
│   │   ├── default_rewrite.rules    # Immutable baseline rewrite rules
...
│   ├── dispatcher.any               # Main dispatcher include file
│   ├── cache/
│   │   ├── default_invalidate.any   # Immutable flush client allow-list
│   │   ├── default_rules.any        # Immutable baseline cache rules
│   │   ├── marketing_query_parameters.any
│   │   ├── rules.any                # Cache rules (what to cache)
│   ├── clientheaders/
│   │   ├── default_clientheaders.any
│   │   └── clientheaders.any        # Custom forwarded client headers
│   ├── filters/
│   │   ├── default_filters.any
│   │   └── filters.any              # Custom request filter rules
│   ├── renders/
│   │   └── default_renders.any      # Generated backend publish host definitions
│   └── virtualhosts/
│       ├── default_virtualhosts.any
│       └── virtualhosts.any         # Custom Dispatcher-level virtual host matching
└── opt-in/
    └── USE_SOURCES_DIRECTLY         # Flexible-mode marker file
```
...
:::note
In AEMaaCS, files such as `default_*.any`, `dispatcher.any`, `default.farm`, and
`default.vhost` are baseline examples or immutable framework files. Put project changes in
customer files such as `filters.any`, `rules.any`, `clientheaders.any`, custom vhosts, and custom
farms. The Cloud runtime uses the current immutable versions even if you edit them locally.
:::
...
| Hostname in `renders` | `default_renders.any` uses `${AEM_HOST}` and `${AEM_PORT}` from the cloud runtime / SDK | Real publish hostnames |
...
In AEMaaCS, `AEM_HOST` and `AEM_PORT` are supplied by the cloud runtime and by the Dispatcher
SDK launcher for local testing. Do not hardcode publish hostnames in `default_renders.any`.
...
When multiple farms are active, each farm has its own `/virtualhosts` list. Dispatcher farm
selection is order-sensitive: it evaluates farms in their include order from the bottom upward,
then evaluates `/virtualhosts` entries from top to bottom inside each farm. Keep a deterministic
include order (for example `00_default.farm`, `10_site.farm`) and place the broad fallback farm so
it cannot shadow more specific farms.
...
Filter rules are evaluated **top-to-bottom**, but the **last matching rule wins**. That is why the deny-by-default pattern below works: the initial `deny *` matches everything, and later, more specific `allow` rules override it for the paths you want to expose.
...
When TTL is enabled, the Dispatcher evaluates standard expiration headers from the backend,
notably `Cache-Control: max-age=...` and `Expires`. A cached file whose auxiliary TTL marker has
expired is re-fetched regardless of the `.stat` file.
...
| `s-maxage` | Shared caches such as the CDN; validate Dispatcher behavior before relying on it there | `3600` (1 hour) |
| `stale-while-revalidate` | CDN (serves stale while re-fetching) | `60` |

### CDN Integration

In AEMaaCS, a CDN (Fastly) sits in front of the Dispatcher. Cache headers flow through unless a
CDN rule changes them, but the CDN and Dispatcher do not have identical semantics; validate both
layers when using shared-cache directives:
...
| `./bin/docker_run.sh dispatcher/src host.docker.internal:4503 8080` | Start Dispatcher container on port 8080 |
...
An end-to-end check verifies that the Dispatcher can reach the publish tier and that the publish tier responds. AEM ships a lightweight probe at `/libs/granite/core/content/login.html`, but per the `/libs/*` deny-by-default rule above, do not expose it publicly. Allow it through the filter only for this specific path, and restrict *who* can reach it at the Apache level - filter rules have no client-header/IP condition, so that restriction has to live in a `<Location>` block:
...
<Location /libs/granite/core/content/login.html>
    Require ip 10.20.30.0/24   # the load balancer's actual subnet, not a broad RFC1918 block
</Location>
```

Without the `<Location>` restriction, this filter rule alone makes the probe reachable from the public internet. A broad private range like `10.0.0.0/8` is not a real restriction on most networks - scope `Require ip` to the load balancer's actual, narrow subnet. If health checks share the same public listener as normal traffic and you cannot pin down a stable source IP (e.g. a managed CDN with rotating edge IPs), do not rely on IP allow-listing alone: allowlist the CDN's published IP ranges *and* require an exact, secret health-check header the CDN adds for these requests, or terminate the check on a private/dedicated listener that never faces the public internet, or put strict authentication in front of it.

```bash
# From the load balancer:
curl http://dispatcher/libs/granite/core/content/login.html
```
