...
:::warning
On AEMaaCS, **mutable content** (`ui.content`) can be installed by pipelines, but it
must be packaged with safe import modes (typically `merge`) so deployments do not
overwrite author-owned content. Use `repoinit` for deterministic bootstrap paths,
service users, groups, and ACLs.
:::
...
| `/content` | **Mutable** | Installed according to package import mode; authors usually own this |
