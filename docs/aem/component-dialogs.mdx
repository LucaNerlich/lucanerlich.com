...
Following JQuery Example is for a multifield
...
(function (window, document, Granite, $) {
    ...
    $(document).on('dialog-ready', (event) => {
        ...
        // Its plural, since a multifield can contain multiple item containers
        const sources = $('coral-select.some-source');
        if (!sources) {
            return;
        }

        sources.each((index, select) => {
