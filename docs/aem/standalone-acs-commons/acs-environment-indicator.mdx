...
Upstream ACS Commons documents the classic indicator as **AEM 6.5 only** and redundant on
AEM as a Cloud Service because Unified Shell already exposes an environment badge (although the
ACS feature can still be configured there with environment variables). This module takes a
different approach: reuse Unified Shell's badge when present, and render a fallback badge when it
is not.
