...
| `cssProcessor` | String[] | CSS processing pipeline (e.g. `["default:none", "min:yui"]`) |
...
Do not rely on the HTML Library Manager OSGi configuration to set system-wide minifiers in an AEMaaCS full-stack
pipeline. Define `cssProcessor` and `jsProcessor` on each `cq:ClientLibraryFolder` instead.
:::
...
AEM as a Cloud Service generates long-cache-key clientlib URLs with a content hash so browsers can cache aggressively:
...
On the Dispatcher side, ensure your cache rules allow `/etc.clientlibs/` paths. On AEM 6.5/AMS, enable equivalent
versioned-clientlib handling (for example, a long-cache-key transformer) before using long browser TTLs.
...
<jcr:root xmlns:cq="http://www.day.com/jcr/cq/1.0"
          ...
          allowProxy="{Boolean}true"
          categories="[myproject.site]"
          cssProcessor="[default:less, min:yui]"/>
```
...
- [@aemvite: Migrate ui.frontend from Webpack to Vite](./aemvite.mdx) - a full walkthrough of replacing webpack + `aem-clientlib-generator` with Vite while keeping clientlib descriptors byte-identical
