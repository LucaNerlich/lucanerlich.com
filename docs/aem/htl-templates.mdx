...
:::caution
For security, validate the element name in your Sling Model and expose only a small allowlist
(for example `h2` through `h4`). HTL also filters invalid element names through the XSS protection
service, but your model should not pass arbitrary author input to `data-sly-element`.
:::

### data-sly-include

Includes the output of another HTL file (or server-side script) into the current markup. The included
script runs in the same Sling request/bindings context, but it does not receive typed template
parameters. Use `requestAttributes` for small include-time values, or `data-sly-template` /
`data-sly-call` when you need explicit parameters.
...
<!-- Script string context (inside quoted inline JS strings) -->
<script>var label = "${model.label @ context='scriptString'}";</script>

<!-- Script token context (for pre-serialized JSON from trusted model code) -->
<script>var config = ${model.jsonConfig @ context='scriptToken'};</script>
...
| `attributeName`| For dynamic attribute names                       |
| `elementName`  | For dynamic element names                         |
| `uri`          | For `href` and `src` attribute values             |
| `scriptToken`  | For JavaScript tokens, such as trusted serialized JSON |
...
In AEM, Adobe documents the default AntiSamy policy at:

| Path | Used by |
|------|---------|
| `/libs/cq/xssprotection/config.xml` | AEM XSS filtering, including the CQ/Granite XSS API and HTL output filtering in AEM projects |

Apache Sling runtimes can also provide Sling-level XSS configuration, but for AEM component projects
follow Adobe's AEM guidance and overlay `/libs/cq/xssprotection/config.xml`.
...
- [i18n and Translation](./content/i18n-translation.md) - dictionaries, translation framework
