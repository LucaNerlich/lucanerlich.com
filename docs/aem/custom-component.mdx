...
Still on webpack? [`@aemvite`](./aemvite.mdx) is a drop-in Vite + esbuild toolchain for exactly
this module - it replaces webpack, Babel, PostCSS, ESLint, and `aem-clientlib-generator` while keeping
the emitted clientlib descriptors byte-identical to the archetype's historical output.
...
- [@aemvite: Migrate ui.frontend from Webpack to Vite](./aemvite.mdx) - replace webpack + `aem-clientlib-generator` with Vite for the ui.frontend module described above
