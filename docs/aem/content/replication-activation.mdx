...
Replication agents are the configuration objects that control **where** and **how** content is
replicated on AEM 6.5 and earlier self-managed topologies. Each agent defines a transport URI,
authentication credentials, trigger behavior, and retry settings. On AEM as a Cloud Service,
distribution agents are managed by Adobe infrastructure rather than by project code.
...
public void activatePage(ResourceResolver resolver, String path) throws ReplicationException {
    ...
    if (session == null) {
        throw new ReplicationException("Cannot adapt resolver to JCR Session");
    }
...
### Using the Tree Activation UI (AEM 6.5)

1. Navigate to **Tools → Replication → Tree Activation**. In AEM as a Cloud Service, use
   **Manage Publication** or the **Publish Content Tree** workflow instead.
...
public void activateTree(ResourceResolver resolver, Replicator replicator, String rootPath)
    ...
    if (session == null || pageManager == null) {
        throw new ReplicationException("Cannot adapt resolver to required AEM APIs");
    }
    ...
    // Activate root
    replicator.replicate(session, ReplicationActionType.ACTIVATE, root.getPath());

    // Recursively activate children
    Iterator<Page> children = root.listChildren(new PageFilter(false, false));
...
:::tip
For large subtrees (thousands of pages), prefer **Manage Publication** or the **Publish Content Tree**
workflow. On AEM as a Cloud Service these replace the classic Tree Activation tool and avoid custom
HTTP request timeouts.
:::

## Reverse Replication

Reverse replication moves content in the opposite direction - from **publish** back to **author**.
This is a legacy AEM 6.5 pattern for content that originates on the publish tier, such as
user-generated content (UGC), form submissions, or community data. It is not a recommended pattern
for AEM as a Cloud Service; use an external service or data store for publish-originated data.
...
| **Flush** | Flush agent to Dispatcher | Adobe-managed Dispatcher/CDN invalidation |
...
7. **Missing Dispatcher flush (AEM 6.5)** - Content may be activated on publish but visitors still see the old version because the Dispatcher cache was not invalidated. Ensure flush agents are configured and healthy. On AEM as a Cloud Service, verify the publication event and Adobe-managed CDN/Dispatcher invalidation instead.
