...
| Multi-line text | Object with `html`, `plaintext`, `markdown`, `json` sub-fields | Choose the format via sub-selection |
| Number (Integer) | `Int` | |
| Number (Float) | `Float` | |
| Boolean | `Boolean` | |
...
```bash
# Local SDK / dev only - admin:admin is fine against the local author. In real environments
# authenticate the CI job that publishes persisted queries with a scoped service account.
...
| `/graphql/execute.json/{config}/{query-name};var1=val1;var2=val2` | Execute with variables; URL-encode names and values |
...
| **Dispatcher** | File-based cache (`.stat` invalidation) | Flush agent on AEM 6.5; Adobe-managed invalidation on AEMaaCS |
| **AEM Publish** | In-memory query cache | Automatic on content change |

### Cache invalidation on publish

On AEM 6.5, configure a Dispatcher flush agent so Content Fragment publication invalidates cached
GraphQL responses:
...
On AEM as a Cloud Service, publication and Sling Content Distribution trigger Adobe-managed
Dispatcher/CDN invalidation; do not configure custom flush agents on the cloud service.
...
| Cache not invalidating | On AEM 6.5, ensure Dispatcher flush covers `.json`; on AEMaaCS, verify publication/distribution and cache headers |

## External Resources

- [AEM GraphQL API Documentation](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/headless/graphql-api/content-fragments)
- [CORS Configuration](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/headless/deployment/cross-origin-resource-sharing)
- [Referrer Filter Configuration](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/headless/deployment/referrer-filter)
- [GraphiQL IDE Tutorial](https://experienceleague.adobe.com/en/docs/experience-manager-learn/getting-started-with-aem-headless/graphql/multi-step/explore-graphql-api)
- [AEM Headless Developer Portal](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/headless/introduction)
- [Persisted Queries Documentation](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/headless/graphql-api/persisted-queries)
...
- [Edge Delivery Services](../edge-delivery/overview.mdx)
