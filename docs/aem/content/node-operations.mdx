...
### Move with JCR `Session` (transient)
...
### Copy with JCR `Workspace` (immediate)
...
### Move or copy with Sling `ResourceResolver`

```java
import org.apache.sling.api.resource.ResourceResolver;

public void moveOrCopyResource(ResourceResolver resolver) throws Exception {
    resolver.move("/content/mysite/en/old-page", "/content/mysite/en/new-page");
    resolver.copy("/content/mysite/en/source-page", "/content/mysite/en/copy-of-source");
    resolver.commit();
}
```

:::tip
`session.move()` is transient - it requires `session.save()` to persist. `workspace.copy()` is
immediate and does not require a save. Sling `resolver.move()` and `resolver.copy()` are persisted
with `resolver.commit()`. Copy duplicates the entire subtree including child nodes and properties.
:::
...
/**
 * Walk up the page hierarchy from the current page and return the first
 * `jcr:content/customNode` that exists, or null if none is found.
 * Useful for reading inherited configuration set via a dialog on an ancestor page.
 */
private Resource getCustomNodeResource(Page page) {
    ...
    while (page != null) {
        Resource resource = resolver.getResource(page.getPath() + childPath);
        if (resource != null) {
            return resource;
        }
        page = page.getParent();
...
| **Copy**                 | `resolver.copy()`                | `workspace.copy()`               |
...
- [Apache Sling Resources](https://sling.apache.org/documentation/the-sling-engine/resources.html) - official Sling Resource API reference
- [JCR 2.0 API (Jackrabbit)](https://jackrabbit.apache.org/jcr/jcr-api.html) - javax.jcr specification and javadoc
- [Repository Structure Guidelines (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/platform/sling-cheatsheet) - Sling cheatsheet and resource resolution
