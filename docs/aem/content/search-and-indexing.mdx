...
|------|-----------|----------|
| **Property index** | Oak property index | AEM 6.5/simple local exact-match lookups on one/few properties |
...
In modern AEM you almost always create a **Lucene** index. AEM as a Cloud Service custom index
definitions must be `type="lucene"`; pure Oak property indexes are only appropriate for AEM 6.5 or
local/on-premise scenarios where you control the repository directly.

## Defining a property index (AEM 6.5 / local only)
...
- Customize an OOTB index: include the product index version and append `-custom-<N>`, e.g.
  `damAssetLucene-8-custom-1`.
- Brand-new index: use a namespace prefix and product/custom versions, e.g.
  `acme.product-1-custom-1`, and include `indexRules`.
...
- **Query Performance** - `/libs/granite/operations/content/diagnosistools/queryPerformance.html`
  lists slow and popular queries; on AEMaaCS use the Developer Console view.
- **Explain Query** - the **Explain Query** tab in Query Performance shows which index a query uses
  (or reports a traversal) and the estimated cost. Run every new query through it before shipping.
- **Index Manager** - available on local SDK/AEM 6.5 only; AEMaaCS production index status is exposed
  through Cloud Manager and the Developer Console rather than instance-local index management.
