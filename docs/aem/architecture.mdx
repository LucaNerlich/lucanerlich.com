...
The core philosophy is **everything is content**. Pages, components, templates, configurations, and rendering
scripts all live as nodes in the JCR. This single content tree drives URL resolution, rendering, and access control.
...
| **Dispatcher** | Apache httpd module (or CDN edge) that caches rendered pages. Invalidated via flush agents on 6.5/AMS or managed invalidation on AEMaaCS |
...
3. On AEM 6.5/AMS, the replication agent serialises the content and pushes it to one or more **Publish** instances.
   On AEMaaCS, Adobe-managed content distribution performs the equivalent movement.
4. A 6.5/AMS flush agent, or the managed AEMaaCS invalidation flow, invalidates the **Dispatcher/CDN** cache for the
   affected paths.
...
:::note[Legacy: WCMUsePojo]
Older codebases may still use `WCMUsePojo` (extend the class, override `activate()`, look up
properties manually). It predates Sling Models, has no dependency injection, is harder to unit
test, and should not be used in new code. If you inherit a project that uses it, migrate to Sling
Models as components are touched.
:::
...
```bash
# Local SDK / dev only - never use admin:admin credentials outside a local instance.
...
- **Closing a request-scoped resolver** - resolvers injected via `@SlingObject` or `request.getResourceResolver()` are managed by Sling and closed automatically at the end of the request. Do **not** close them yourself - doing so breaks other components that still need the resolver during the same request
...
- **Not closing manually opened resolvers** - service resolvers obtained via `getServiceResourceResolver()` are **your** responsibility. Always use try-with-resources. A leaked resolver means a leaked JCR session
...
- **Not invalidating on publish** - on 6.5/AMS, ensure flush agents are correctly configured; on AEMaaCS, inspect the
  managed invalidation/CDN headers before assuming the content did not publish
