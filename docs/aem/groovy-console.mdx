...
| `distributedExecutionEnabled` | Enable distributed execution for scripts replicated from author to publish/preview | `false` |
...
| `out` | `PrintStream` | Writes output to the console result panel |
| `slingRequest` | `SlingHttpServletRequest` | HTTP request object, available for web UI / HTTP executions |
| `slingResponse` | `SlingHttpServletResponse` | HTTP response object, available for web UI / HTTP executions |
| `nodeBuilder` | `NodeBuilder` | Groovy DSL builder for JCR node hierarchies |
| `pageBuilder` | `PageBuilder` | Groovy DSL builder for AEM page hierarchies |

### Built-in helper methods

The console also adds convenience methods to the script class:

| Method | Description |
|--------|-------------|
| `getService(Class)` / `getService(String)` | Return one OSGi service |
| `getServices(Class, String)` / `getServices(String, String)` | Return OSGi services matching an LDAP filter |
| `getResource(String)` | Resolve a Sling resource |
| `getPage(String)` | Resolve a CQ page |
| `getNode(String)` | Resolve a JCR node |
| `save()` | Save the current JCR session |
| `xpathQuery(String)` / `sql2Query(String)` | Execute JCR queries and return node iterators |
| `activate(String)` / `deactivate(String)` | Replicate or deactivate a path |
| `table { ... }` | Render structured tabular output in the Table tab |

You can retrieve any OSGi service via the helper method or via `bundleContext`:

```groovy
import com.day.cq.replication.Replicator

// Option 1: via Groovy Console helper
...
while (true) {
    ...
    while (rows.hasNext()) {
        ...
        if (!DRY_RUN) {
            // Use an unprefixed property name; a prefix like 'demo:' requires
            // the namespace to be registered on the JCR NamespaceRegistry first.
            node.setProperty('lastScanned', java.time.Instant.now().toString())
            mutated++
        }
    }

    // Persist each batch instead of accumulating every change in memory
    // (see "Batch your saves") - avoids OutOfMemoryError on large datasets.
    if (!DRY_RUN) s.save()
...
List all pages using a given template, in tree-traversal order:
...
getPage(BASE_PATH).recurse { page ->
    page.node?.recurse { node ->
        ...
        // getResource honours the Sling search paths (/apps then /libs) and
        // handles absolute resourceTypes, so no manual prefixing is needed.
        if (resourceType && !resourceResolver.getResource(resourceType)) {
...
getPage(BASE_PATH).recurse { page ->
    ...
    // (hideInNav is a Boolean property, so check it directly)
    if (page.properties['hideInNav']) {
...
This example updates the assets **directly** under `BASE_PATH`. To process an entire
subtree (assets nested in subfolders), adapt the [batched SQL2 template](#batched-sql2-query-baseline-template)
with `NODE_TYPE = 'dam:Asset'` instead.
...
Stream query results into CSV output. For interactive reporting, the Groovy Console's `table { ... }`
helper renders a sortable grid in the **Table** tab; you can also build the CSV string yourself and print it:
...
// Select only the page path; read jcr:content properties per row below.
// (Oak does not reliably return relative properties such as
// jcr:content/jcr:title in the SELECT list.)
String sql = """
    SELECT p.[jcr:path]
    FROM [cq:Page] AS p
    WHERE ISDESCENDANTNODE(p, '${BASE_PATH}')
...
// Read a property off the page's jcr:content node, or '' if absent
def contentProp = { String pagePath, String name ->
    String contentPath = pagePath + '/jcr:content'
    if (!session.nodeExists(contentPath)) return ''
    def content = session.getNode(contentPath)
    content.hasProperty(name) ? content.getProperty(name).string : ''
}
...
rows.each { row ->
    String path = row.getPath('p')
    def values = [path, contentProp(path, 'jcr:title'), contentProp(path, 'cq:template')]
    // Quote fields and escape embedded quotes to keep the CSV well-formed
    def cells = values.collect { v -> '"' + v.replace('"', '""') + '"' }
...
:::tip[The `table` helper]
For tabular output, use the built-in `table` method instead of assigning to a variable:

```groovy
table {
    columns 'Path', 'Title', 'Template'
    row '/content/my-site/en', 'Home', '/conf/my-site/settings/wcm/templates/content-page'
}
```
:::
...
def agentManager = getService(AgentManager)
