...
group.getMembers().each { Authorizable member ->
    out.println("${member.id}${member instanceof Group ? ' (group)' : ''} -> ${member.path}")
    count++
}
...
getPage(BASE_PATH).recurse { page ->
    page.node?.recurse { node ->
        node.properties.each { name, value ->
            // Only check single-value string properties that look like a content/dam path
            if (value instanceof String && value.startsWith('/content/')) {
                ...
                if (!session.nodeExists(path) && !session.nodeExists(value)) {
                    out.println("BROKEN: ${node.path}@${name} -> ${value}")
                    broken++
                }
            }
        }
...
resourceResolver.getResource(BASE_PATH).listChildren().each { child ->
    ...
    // referenceSearch via QueryBuilder: any node whose property equals this asset path
    def params = [
        'path'          : '/content',
        'property'      : '*',
        'property.value': child.path,
        'p.limit'       : '1'
    ]
    ...
    if (result.totalMatches == 0) {
        out.println("ORPHAN: ${child.path}")
        orphans++
    }
}

out.println('----------------------------------------')
out.println("Orphaned assets (no references found): ${orphans}")
return orphans
```

:::warning "Unreferenced" is not "safe to delete"
An asset may be referenced from external systems, hardcoded URLs, or rich-text HTML this query does
not scan. Always review the list (and back up) before deleting.
:::
