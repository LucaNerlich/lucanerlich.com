...
| Asset ingestion and processing | DAM Update Asset (AEM 6.5) / Asset microservices (AEMaaCS) | Photographer/Designer, DAM Admin |
    ...
    public void execute(WorkItem workItem, WorkflowSession workflowSession,
        ...
        if (properties != null) {
            ...
            try {
                ...
                throw new WorkflowException("Could not save tags for " + payloadPath, e);
            }
        }
    }
}
...
An OR Split routes execution to one of several branches based on a condition. The typical pattern is:

1. A **Process Step** before the OR Split writes a boolean variable into the workflow's
   `MetaDataMap`.
2. Each OR Split branch has a **Script** (or **Rule**) expression that reads the variable
   and returns `true` / `false`.

The Process Step is a normal `WorkflowProcess`:

```java title="core/.../workflow/PublishReadyCheckStep.java"
package com.myproject.core.workflow;
...
@Component(
    service = WorkflowProcess.class,
    property = {
        "process.label=My Project - Set Publish Ready Flag"
    }
)
public class PublishReadyCheckStep implements WorkflowProcess {
    ...
    public void execute(WorkItem workItem, WorkflowSession session, MetaDataMap args)
        ...
        Object payload = workflowData != null ? workflowData.getPayload() : null;
        if (payload == null || !"JCR_PATH".equals(workflowData.getPayloadType())) {
            throw new WorkflowException("Expected a JCR_PATH payload");
        }
        ...
        if (jcrContent != null) {
            ValueMap props = jcrContent.getValueMap();
            publishReady = props.containsKey("jcr:title")
                && props.containsKey("jcr:description")
                && props.containsKey("featuredImage");
        }

        // Store the result so the OR Split branches can read it
        workItem.getWorkflow().getWorkflowData().getMetaDataMap()
            .put("publishReady", publishReady);
    }
}
```

In the OR Split, the branches must be complementary, not identical. Give the "ready" branch a
routing expression type of **Script**, with the language set to **ECMA Script**, and paste the
function into the inline script field:

```text
function check() { return workflowData.getMetaDataMap().get("publishReady", false); }
```

Mark the other branch as the **Default** route (taken when no other branch's condition matches),
so exactly one branch fires. If you need an explicit condition on both sides instead of a default,
negate the check on the second branch: `return !workflowData.getMetaDataMap().get("publishReady", false);`

**Script Path** is a separate option that points at a repository node holding the same code as
an external `.ecma` file, rather than accepting inline text.
 ...
 */
@Reference
private WorkflowLauncher launcherService;

public void disableAllLaunchers() throws WorkflowException {
    Iterator<ConfigEntry> entries = launcherService.getConfigEntries();
    while (entries.hasNext()) {
        ConfigEntry entry = entries.next();
        if (entry.isEnabled()) {
            entry.setEnabled(false);
            launcherService.editConfigEntry(entry.getId(), entry);
 ...
 */
public void enableAllLaunchers() throws WorkflowException {
    Iterator<ConfigEntry> entries = launcherService.getConfigEntries();
    while (entries.hasNext()) {
        ConfigEntry entry = entries.next();
        if (!entry.isEnabled()) {
...
- **Post-processing workflows** run after the microservices have finished and can still use custom
Java steps (metadata, tagging, notifications, moving assets) - but should not generate renditions

If you are building for AEMaaCS, plan your DAM customisations around Processing Profiles, Asset
Compute, and post-processing workflows rather than extending DAM Update Asset. See
[Asset Processing on AEMaaCS](../content/asset-processing.mdx) for the full picture.
:::
...
|---------|----------------------|------------------------|
| Custom DAM processing steps | Supported (in DAM Update Asset) | Not in the ingestion pipeline - use processing profiles / Asset Compute for renditions; run custom steps in a post-processing workflow |
| Workflow model editor | Available | Available |
| Custom process steps | Supported | Supported (including post-processing workflows for assets) |
...
- [Adobe: Administering Workflows](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/sites/administering/operations/workflows)
- [Adobe: Developing and Extending Workflows](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/sites/administering/operations/workflows)
- [Adobe: Asset Microservices (AEMaaCS)](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/assets/manage/asset-microservices-configure-and-use)
...
- [acs.workflow - Workflow Package Manager](../standalone-acs-commons/acs-workflow.mdx) - multi-path workflow payloads without full ACS Commons
