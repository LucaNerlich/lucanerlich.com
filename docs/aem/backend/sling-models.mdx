...
```text title='file structure / aem component'
/some.html
/helper.js
```
...
- [Annotations](../components/annotations/child-resource.mdx)
...
```javascript title='JavaScript Use Model'
use(function () {
  'use strict';

  var someChangedValue = this.someVar;
  // to what ever you want with `this.someVar` which is `someValue` passed in.
  // ⚠️ this code will be executed server-side, console.log, window etc do not exist
  // use ES5. ES6 will fail.

  return {
    someVar: someChangedValue
  }
});
```

```html title='HTL Template'
<div data-sly-use.helper="${'helper.js' @ someVar=someValue}">
${helper.someVar}
</div>
```
