...
AEM provides three event APIs for reacting to changes in the repository and the platform. Understanding
...
:::warning[AEMaaCS]
JCR observation listeners are discouraged on AEM as a Cloud Service. Prefer
`ResourceChangeListener` for repository changes and offload durable work to Sling Jobs.
:::
...
public class JcrNodeListener implements EventListener {
    ...
    protected void activate() throws LoginException, RepositoryException {
        resolver = resolverFactory.getServiceResourceResolver(
            Map.of(ResourceResolverFactory.SUBSERVICE, "my-service-user"));
        session = resolver.adaptTo(Session.class);
        if (session == null) {
            throw new RepositoryException("Could not adapt service resolver to Session");
        }
...
public class IndexUpdateJobConsumer implements JobConsumer {
    ...
    public JobResult process(Job job) {
        ...
        try (ResourceResolver resolver = resolverFactory.getServiceResourceResolver(
            ...
            // Return FAILED to let the queue retry (see queue.retries below), or
            // CANCEL for errors you know are not worth retrying (e.g. bad input).
...
Retry behaviour is **not** configured on the consumer - it lives on the **queue** that processes
the topic. Deploy a `QueueConfiguration` factory config that matches your topic:

```json title="ui.config/.../config/org.apache.sling.event.jobs.QueueConfiguration~index-update.cfg.json"
{
    "queue.name": "Index Update Queue",
    "queue.topics": ["com/myproject/job/INDEX_UPDATE"],
...
See [Sling Jobs and Schedulers](./sling-jobs.md) for the full list of queue properties.
...
Navigate to `http://localhost:4502/system/console/slingevent` to see:
...
- [Sling Jobs and Schedulers](./sling-jobs.md) - guaranteed background processing
- [Security](../infrastructure/security.mdx)
- [Groovy Console](../groovy-console.mdx) - testing event handlers
- [Apache Sling Eventing and Job Handling](https://sling.apache.org/documentation/bundles/apache-sling-eventing-and-job-handling.html) - OSGi EventAdmin and Sling event handling
- [JCR Observation API (Jackrabbit Oak)](https://jackrabbit.apache.org/oak/docs/apidocs/org/apache/jackrabbit/api/observation/package-summary.html) - low-level JCR observation reference
- [AEM Event Handling (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/aem-eventing/overview) - AEMaaCS cloud-native eventing
