...
- Prefer Sling Jobs for work that must run once in a cluster; use the Sling Scheduler only as the trigger.
...
Use Sling's `org.apache.sling.xss.XSSAPI` to validate and encode user input before it reaches
templates or logs.
...
public String safeText(String input) {
    // Encode for HTML element text. Use a context-specific encoder for attributes or JS.
    return xssApi.encodeForHTML(input == null ? "" : input);
}

public String safeUrl(String url) {
    // Sanitizes href/src values; invalid URLs return an empty string.
