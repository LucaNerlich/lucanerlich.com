...
When the type is ambiguous, add the Sling Installer type suffix to the property name:
`"connectionTimeoutMs:Integer": 5000` or `"allowedMimeTypes:String[]": ["image/jpeg"]`.
...
### Annotation method name to property name mapping

The generated metatype property ID is derived from the annotation method name. Use underscores when
the property name contains characters that are not valid in Java method names:

| Method name | Configuration property |
|-------------|------------------------|
| `scheduler_expression()` | `scheduler.expression` |
| `service_ranking()` | `service.ranking` |
| `my__property()` | `my_property` |
| `my$_$property()` | `my-property` |
| `cost$$center()` | `cost$center` |

Most project-specific properties can stay camelCase (`apiBaseUrl()` -> `apiBaseUrl`).
...
Use **Cloud Manager environment variables** for non-secret values and **secrets** for credentials.
Reference them with `$[env:...]` or `$[secret:...]` placeholders:
...
- [OSGi Metatype Specification](https://docs.osgi.org/specification/osgi.cmpn/8.1.0/service.metatype.html) - `@ObjectClassDefinition` / `@AttributeDefinition` spec
- [OSGi Configuration Admin Specification](https://docs.osgi.org/specification/osgi.cmpn/8.1.0/service.cm.html) - configuration binding and factory PIDs
- [AEM OSGi Configuration (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/deploying/configuring/configuring-osgi) - AEM-specific guidance
- [AEMaaCS Environment Variables](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/environment-variables) - Cloud Manager secret/env vars
