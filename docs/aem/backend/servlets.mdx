...
A servlet that handles contact form submissions. See [CSRF Protection](#csrf-protection) below for
the token handling AEM adds on top:
...
```bash
# Local SDK / dev only - never ship admin:admin credentials against a real environment.
...
protected void doGet(SlingHttpServletRequest request,
    ...
    String normalizedPath = ResourceUtil.normalize(targetPath);
    if (StringUtils.isBlank(normalizedPath)
            || !normalizedPath.startsWith("/content/myproject/")) {
        response.sendError(SlingHttpServletResponse.SC_BAD_REQUEST, "Invalid target parameter");
        return;
    }

    // Use ResourceResolver.map() for proper URL mapping after allowlisting the target.
    String mappedUrl = request.getResourceResolver().map(request, normalizedPath);
...
Dispatcher can cache GET responses when cache rules allow the URL. For servlets that return dynamic
data, configure Dispatcher/CDN cache rules explicitly and send browser cache headers that match the
intended behaviour.
...
The `Cache-Control` response header does not by itself override every Dispatcher configuration. Make
sure the Dispatcher `/cache/rules` and `/cache/headers` settings for the endpoint match your intent.
...
// Validate after normalization: must remain under /content/
...
- [Apache Sling Servlets](https://sling.apache.org/documentation/the-sling-engine/servlets.html) - official servlet registration reference
- [Sling Servlet Annotations](https://sling.apache.org/documentation/the-sling-engine/servlets.html#servlet-registration) - `@SlingServletResourceTypes` / `@SlingServletPaths`
- [AEM Servlets (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-learn/getting-started-wknd-tutorial-develop/project-archetype/custom-component) - tutorial walkthrough
