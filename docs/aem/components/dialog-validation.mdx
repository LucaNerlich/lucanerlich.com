...
- [Using Validation in Granite UI](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/clientlibs/foundation/js/validation/index.html) - the `foundation.validation` API (`validator`, `selector`, `validate/show/clear`)
- [Granite UI `field` component](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/field/index.html) - the `foundation-field` adapter the validators call
- [TextField `validation` attribute](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/textfield/index.html) - how a field declares a validator
- [Creating a New Granite UI Field Component](https://experienceleague.adobe.com/en/docs/experience-manager-65-lts/content/implementing/developing/platform/granite-ui-component)

The validation framework is **identical on AEM 6.5 and AEM as a Cloud Service** - the same
`foundation-registry` API, the same field attributes, and the same `cq.authoring.dialog` clientlib
category. Nothing in this guide is platform-specific.
:::
...
<myField
    ...
    <granite:data
        jcr:primaryType="nt:unstructured"
        foundation-validation="my-custom-rule"
        max-length="100"/>
</myField>
```
...
<granite:data
    jcr:primaryType="nt:unstructured"
    foundation-validation="file-extension"
...
<description
    ...
    <granite:data
        jcr:primaryType="nt:unstructured"
        foundation-validation="char-count"
        max-chars="160"/>
</description>
```
...
<altText
    ...
    <granite:data
        jcr:primaryType="nt:unstructured"
        foundation-validation="conditional-required"
...
(function ($) {
    ...
    $(window)
        .adaptTo("foundation-registry")
        .register("foundation.validation.validator", {
            selector: "[data-validation~='conditional-required'], [data-foundation-validation~='conditional-required']",
...
Client-side async checks are a UX optimisation, not a security boundary. Re-validate critical rules in
the server endpoint that performs the operation, in a workflow/process step, or in another trusted
post-processing layer. A Sling Model `@PostConstruct` can detect and render warnings later, but it does
not block the dialog POST that already saved content. See [Server-Side Validation](#server-side-validation)
below.
:::
...
For critical business rules, add server-side checks. A Sling Model can detect invalid saved content and
surface an authoring warning, but it cannot reject the original dialog save:
...
| **Server-side endpoint / workflow / listener** | Enforce rules for custom operations or repair/reject invalid content after save | Hard only when it owns the write or rollback path |
| **Sling Model** | Detect invalid persisted content and expose warnings during render/export | Diagnostic - does not block saving |
| **Content policy** | Template-level authoring constraints | UI/configuration guardrail - can still be bypassed by direct repository writes |
...
A validation error that is only communicated with a red border is invisible to screen-reader users.
Calling `fieldAPI.setInvalid(true)` already does most of the work - Coral sets `aria-invalid="true"`
on the control - but make the rest of the experience accessible too:

- **Always show a text message**, never colour alone. The Coral error `Tooltip` you create in `show()`
  carries the message text; keep it specific (the WCAG "error identification" success criterion).
- **Associate the message with the field.** Give the tooltip an `id` (the examples already use
  `Coral.commons.getUID()`) and point the field at it with `aria-describedby` so assistive tech reads
  the error together with the label:

```javascript
show: function (el, message) {
    // ... build the Coral.Tooltip with error.id = Coral.commons.getUID() ...
...
- **Move focus to the first invalid field** on submit so keyboard users land on the problem rather
  than hunting for it. `checkValidity()` plus `el.focus()` on the first failure is enough.
- **Don't disable the submit button** to "prevent" invalid saves - a disabled control gives no
  feedback about *why*. Let validation run and report the error instead.

See the [Granite UI `field` accessibility contract](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/field/index.html)
and the project's [Component Dialogs accessibility notes](../component-dialogs.mdx#accessibility).
