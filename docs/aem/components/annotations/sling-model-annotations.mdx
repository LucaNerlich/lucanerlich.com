...
Java field names cannot contain `:`, so JCR properties with namespaces (like `jcr:title` or
`cq:lastModified`) must be mapped via the `name` attribute:
...
public class WrapperModel {
    ...
    // Delegates using the current resource's sling:resourceSuperType
...
When using `@Inject`, Sling Models runs every registered injector in order of its OSGi
`service.ranking`. For historical reasons, **lower** service ranking values are treated as
**higher** priority - see [SLING-5035](https://issues.apache.org/jira/browse/SLING-5035).
The first injector that returns a non-`null` value wins.

The exact order depends on which injectors are installed (including custom ones) and their
configured service rankings, so it is not fixed. This ambiguity is precisely why the
injector-specific annotations (`@ValueMapValue`, `@OSGiService`, `@SlingObject`, etc.) are
preferred - they bypass the priority chain entirely and target one injector directly.
...
// Read a property from a specific child resource path
