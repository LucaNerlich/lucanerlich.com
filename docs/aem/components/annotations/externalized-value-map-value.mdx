...
public class ExternalizedValueMapValueInjector
    ...
    public Object getValue(Object adaptable, String name, Type type,
        ...
        if (!annotatedElement.isAnnotationPresent(ExternalizedValueMapValue.class)) {
            return null;
        }

        // Resolve the request and resource from the adaptable
...
If you use `@ExternalizedValueMapValue` in your Sling Model, don't call `resolver.map()` or
`Externalizer` again in HTL or another getter. The path is already mapped. HTL URI context is escaping,
not URL mapping; using `${model.linkUrl}` in an `href` attribute (or an explicit `@ context='uri'` when
needed) is still appropriate.
...
- [Apache Sling Models Injectors](https://sling.apache.org/documentation/bundles/models.html#available-injectors) - how injectors and `@Source` work
- [AEM Externalizer (Experience League)](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/platform/externalizer) - official `com.day.cq.commons.Externalizer` reference
- [Sling URL Mapping](https://sling.apache.org/documentation/the-sling-engine/url-decomposition.html) - how `ResourceResolver.map()` resolves URLs
