...
at `/libs/core/wcm/components/image/v2/image/cq:dialog`. Copy/paste this and remove the 'asset' and 'metadata' tabs, since
...
Using this method, all properties will be written directly to the component's `jcr:content` node and need to be handled
manually in your Sling Model. As of now, no reusable OSGi service exists to delegate the logic to.
