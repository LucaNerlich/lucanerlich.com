...
![Core Components release artifacts](/images/aem/core-components/github-release.jpg)
...
If you are running an AEM Cloud SDK, the Core Components are bundled with the SDK and available under `/apps/core/wcm/components/` - you do not need to install them separately on AEMaaCS.

#### Core Components - Manual Installation

If you want to add the Core Components to your development environment and install them via Maven / `pom.xml`, a few
additions are required. First, add a version variable, then add the required POM dependencies. These
need to be added to the root POM, the `all` POM, the `core` POM, and the `ui.apps` POM.

If you bootstrap your development environment via the Maven AEM Project Archetype, this will have already been done for you.

:::tip[Check for the latest version]
Always verify the current release at the [Core Components GitHub Releases](https://github.com/adobe/aem-core-wcm-components/releases) page before pinning a version - the examples below use a recent release but a newer one may be available.
:::

##### Root POM

```xml title="/pom.xml"
<core.wcm.components.version>2.32.8</core.wcm.components.version>
...
```xml title="/all/pom.xml - filevault-package-maven-plugin <embeddeds>"
<embeddeds>
    <embedded>
        <groupId>com.adobe.cq</groupId>
        <artifactId>core.wcm.components.content</artifactId>
...
> Based on current Core Components releases where the Image component resource type is `image/v3/image`.
...
Extending via `sling:resourceSuperType` is as simple as referencing the parent in the component's `.content.xml` definition.
...
You then need to look at the original dialog structure, which can be found
at `/libs/core/wcm/components/image/v3/image/cq:dialog`. Copy/paste this and remove the 'asset' and 'metadata' tabs, since
these will be served from the inherited parent. You can now add your own dialog tab instead. The resulting dialog will
merge all tabs together. Individual tabs can be hidden via `sling:hideChildren` on the tabs 'items' node.
...
Using this method, all properties will be written directly to the component's `jcr:content` node. For a simple proxy
or extension of a Core Component, prefer the standard Sling Model delegation pattern with
`@Self @Via(type = ResourceSuperType.class)` so the Core Component model keeps handling its own logic
(see [Extending Core Components with Delegation](./core-components-delegation.mdx)).
...
Prefer a standalone module instead of full ACS Commons? Use
[acs.include](../standalone-acs-commons/acs-include.mdx) from
[standalone-acs-commons](https://github.com/LucaNerlich/standalone-acs-commons). Migration from ACS Commons
requires replacing `acs-commons/granite/ui/components/include` with
`acs-include/granite/ui/components/include` and removing the leading slash from every `path` value (for example
`path="/libs/..."` becomes `path="libs/..."`). After that, placeholder/parameter authoring matches ACS Commons,
with optional `hide` and typed casting on top.
            ...
            <tabs
                ...
                <items jcr:primaryType="nt:unstructured">
                    <image
                        ...
                        path="/libs/core/wcm/components/image/v3/image/cq:dialog/content/items/tabs/items/asset"
                        namespace="image"
                        margin="{Boolean}true"/>
                    <imageMetaData
                        jcr:primaryType="nt:unstructured"
                        jcr:title="MetaData"
                        sling:resourceType="acs-commons/granite/ui/components/include"
                        path="/libs/core/wcm/components/image/v3/image/cq:dialog/content/items/tabs/items/metadata"
                        namespace="image"
                        margin="{Boolean}true"/>
                    <ctaLinksContainer
                        jcr:primaryType="nt:unstructured"
...
![Extended image root node](/images/aem/core-components/extended-image-root.jpg)

Due to the parameterized namespace include, all included/referenced image v2 dialog nodes will be included in
the `namespace="image"`

![Extended image namespace](/images/aem/core-components/extended-image-image.jpg)
...
![Extended image CTA links](/images/aem/core-components/extended-image-ctalinks.jpg)
...
![Original image tab](/images/aem/core-components/extended-image-tab-original.jpg)

![Custom image tab](/images/aem/core-components/extended-image-tab-custom.jpg)
...
We use the Child Resource From Request Injector to enable the injected object to access a Sling request. This lets the
Image Core Component correctly populate all its fields.
...
This lets us build custom components while reusing existing Core Components. Projects often need complex
components that combine images, text, checkboxes, and other fields - this pattern makes that possible while
...
This example uses Lombok annotations to avoid getter/setter boilerplate in simple Java POJOs.

By injecting our `image` object with the aforementioned ACS injector, we leverage the full Core Components
Image Sling Model and any logic it applies. We can inject `ctaLinks` (see Part 1 for the dialog XML) via a
regular `@ChildResource`, since we are passing the untouched JSON data from the JCR to the `.model.json` output.
...
Implementing the Sling Model above together with the component from Part 1 gives us a custom `extendedImage`
component that exports all Core Components Image properties under the `image` namespace, along with our own
...
- [Extending Core Components with Delegation](./core-components-delegation.mdx)
