...
When AEM renders an included editable component and decoration is requested, it can wrap the component's own
output in an extra HTML element called the **decoration tag**. This is the element you see around your markup
with a class that matches the component name, for example:
...
The outer `<div class="text ...">` is **not** produced by your HTL. AEM's decoration mechanism adds it when a
container or include asks for decoration (for example editable child components in the Page Editor), so the
...
Separately, the component node itself can set `cq:noDecoration="{Boolean}true"` to suppress wrapper generation
even when an include requests decoration.
...
```html title="Code-reuse include: no decoration element is rendered"
<sly data-sly-resource="${ 'content' @
    resourceType='myproject/components/text' }"></sly>
```

A plain HTL include like this one renders the child's markup without a wrapper. To guarantee that a component
is **never** decorated, even when a container or an include asks for decoration with `decoration=true`, set
`cq:noDecoration` on the component definition instead:

```xml title="ui.apps/.../components/text/.content.xml"
<?xml version="1.0" encoding="UTF-8"?>
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0"
          xmlns:cq="http://www.day.com/jcr/cq/1.0"
          jcr:primaryType="cq:Component"
...
:::warning
Without a decoration element, the Page Editor has no wrapper to attach its overlay to, so a
`cq:noDecoration` component cannot be selected or edited on its own. Use it only for components that are
rendered for code reuse, not for components authors drop into a layout container.
:::

:::tip[HTL includes do not decorate by default]
Plain HTL code-reuse includes such as `data-sly-resource="child"` do **not** render a wrapper by default.
Use `decoration=true`, `decorationTagName`, or the Page Editor's container includes when the child must be
editable or needs layout classes. Empty decoration tags are also suppressed automatically when the component
renders no output.
:::
