...
- One or more self-hosted **Coolify** v4 instances with the API enabled and an API token with read-only access
...
Keep `config.json` out of dotfile sync or source control unless the file is encrypted separately.
