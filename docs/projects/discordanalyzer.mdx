...
Place the downloaded `.json` files in a local directory such as `logs/<name>` that is ignored by Git.
Update the input directory in `readLogs`; avoid placing private exports in a repository.
