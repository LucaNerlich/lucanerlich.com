---
title: "Discord Chat Analyzer"
description: A Java tool that parses Discord chat export JSON files and generates ranked statistics for message count, mentions, reactions, attachments, and the earliest exported messages.
tags: [java, discord, analytics, open-source]
keywords:
...
The DCA operates on static JSON chat exports, downloaded by the
[DiscordChatExporter](https://github.com/Tyrrrz/DiscordChatExporter).

:::warning[Privacy and authorization]
Chat exports can contain messages, usernames, IDs, attachments, and other personal data. Analyze only data
you are authorized to access, keep the exports out of version control, and remove or anonymize personal data
before sharing a report. Do not expose Discord account tokens to this tool or commit them to scripts.
:::

## Architecture and Output

After parsing all `.json` logs into Java entities, each `Channel` is analyzed by `Analyzer.java`.
...
Place the downloaded `.json` files in a local directory outside the repository, and update the input
directory in `readLogs` accordingly. Keep private exports out of the repository.
...
`ACCOUNT_AGE`, `calculateAccountAgeRanking`, and `joinedServer` are legacy implementation names in these
examples. They refer to the earliest message for each author in the export, not account age or server-join date.
...
![Rankings](/images/projects/dca/rankings.png)
...
private static List<Channel> parseJsonToChannels() {
        ...
        final List<Channel> channels = Collections.synchronizedList(new ArrayList<>());
...
### Earliest message in the export

> Author with the earliest message represented in the exported data.

This is **not** the account's creation date or necessarily the date they joined the server. It is limited by
the exported channels and the available history.
...
![Discord Chat Analyzer UML class diagram](/images/projects/dca/uml.png)
