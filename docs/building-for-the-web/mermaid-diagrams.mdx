...
configuration can differ between Docusaurus, GitHub, GitLab, and editor extensions. Keep each Mermaid
definition and its adjacent `markdown` source example identical, then run this site's production build
before publishing:
...
`pnpm build` compiles the page and recognizes Mermaid blocks, but it does not validate their diagram
definitions or parse ordinary `markdown` source examples. Inspect each diagram in the published renderer.
When you update a diagram, keep its adjacent `markdown` source example identical in the same change.
